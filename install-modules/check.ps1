...
.DESCRIPTION
    ...
    The tests in test.ps1 run after the analyzer and need nothing fetched.
    They build a synthetic shed in a temporary directory and install into
    another one, so running this check never writes into the module
    directories the tool is for. Where symbolic links cannot be created,
    which needs Developer Mode or an elevated shell on Windows, the link
    cases report as skipped rather than passing quietly.
