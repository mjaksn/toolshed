...
<#
.SYNOPSIS
    DispatchDesk. Creates Windows desktop shortcuts that run GitHub Actions
...
.DESCRIPTION
    ...
    Requirements:
      ...
      - The powershell-yaml module, which the command offers to fetch on first run
#>

$script:ShortcutRoot = Join-Path $HOME '.github_shortcuts'
$script:ActionsDir   = Join-Path $script:ShortcutRoot 'actions'
$script:LogsDir      = Join-Path $script:ShortcutRoot 'logs'
$script:CreatedFiles = @()
...
function Stop-WithError {
    ...
    if ($removed.Count -gt 0) {
        $all += 'Partially created files were removed:'
        $all += @($removed | ForEach-Object { "  $_" })
    }
    throw ($all -join [Environment]::NewLine)
}
...
$script:YamlSha256  = 'd4602bc7a4a093766520422d53ca8b09acde162286fae11e2ee6c8edfea07810'
...
function Initialize-YamlModule {
    ...
    # whatever else is on the machine is neither used nor disturbed. The import
    # lands in this module's own session state, which is where ConvertFrom-Yaml
    # is called from and is why the caller's session never sees it.
    $loaded = @(Get-Module -Name powershell-yaml)[0]
    if ($loaded -and $loaded.Version -eq [version]$script:YamlVersion) { return }

    $root = Join-Path ([System.IO.Path]::GetTempPath()) "powershell-yaml-$($script:YamlVersion)"
    $pkg  = "$root.zip"
    ...
    if (-not (Test-Path -LiteralPath $pkg)) {
        ...
        if (-not (Read-YesNo "Fetch powershell-yaml $($script:YamlVersion) from the PowerShell Gallery now?")) {
            Stop-WithError @(
        ...
        try {
            ...
            Write-Info "Fetching powershell-yaml $($script:YamlVersion)..."
            ...
            Invoke-WebRequest -UseBasicParsing -MaximumRedirection 5 -OutFile $pkg `
                -Uri "https://www.powershellgallery.com/api/v2/package/powershell-yaml/$($script:YamlVersion)"
        }
        catch {
            Stop-WithError @(
                "Could not fetch powershell-yaml $($script:YamlVersion) : $($_.Exception.Message)",
                'Check the network and any proxy, then run New-WorkflowShortcut again.'
            )
        }
    }
    ...
    if ($actual -ne $script:YamlSha256) {
        Remove-Item -LiteralPath $pkg -Force -ErrorAction SilentlyContinue
        if (Test-Path -LiteralPath $root) {
            Remove-Item -LiteralPath $root -Recurse -Force -ErrorAction SilentlyContinue
        }
        Stop-WithError @(
            "powershell-yaml $($script:YamlVersion) did not match its recorded hash.",
            "  expected $($script:YamlSha256)",
            "  got      $actual",
    ...
    if (-not (Test-Path -LiteralPath $psd1)) {
        try { Expand-Archive -LiteralPath $pkg -DestinationPath $root -Force }
            ...
            Stop-WithError @(
                "Could not unpack powershell-yaml $($script:YamlVersion) : $($_.Exception.Message)",
                "Delete $pkg and $root, then run New-WorkflowShortcut again."
            )
        }
    }
    ...
    try { Import-Module -Name $psd1 -Force -ErrorAction Stop }
        ...
        Stop-WithError @(
            "Could not import powershell-yaml $($script:YamlVersion) : $($_.Exception.Message)",
            "Delete $root and run New-WorkflowShortcut again to unpack it afresh."
        )
    }
    Write-Ok "powershell-yaml $($script:YamlVersion) verified and loaded."
}
...
# =====================================================================
$script:RuntimeTemplate = @'
# Generated by New-WorkflowShortcut (DispatchDesk) on __GENERATED_AT__
# Runs the workflow "__WORKFLOW_DISPLAY__" in __REPO_DISPLAY__ from branch "__BRANCH_DISPLAY__".
# Run New-WorkflowShortcut again to regenerate this file rather than editing it by hand.
...
# =====================================================================
function New-WorkflowShortcut {
    <#
    .SYNOPSIS
    Creates a desktop shortcut that runs a GitHub Actions workflow.

    .DESCRIPTION
    Asks for a repository, workflow, branch and a value or a run time prompt for
    each workflow_dispatch input, verifies every prerequisite against GitHub,
    then writes a runner script pair (.cmd launcher and .ps1 runtime) under
    ~/.github_shortcuts/actions and puts a .lnk shortcut on the desktop that
    launches it.

    Nothing is written until a summary has been shown and confirmed. If writing
    ...
    the three paths that were written.
    ...
    # reach into the caller's console or their error preference.
    [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
    $ErrorActionPreference = 'Stop'

    # Module state outlives a call, so a previous failure must not leave names
    # behind for this one to delete.
    $script:CreatedFiles = @()
    ...
    if (-not ($env:OS -eq 'Windows_NT')) {
        Stop-WithError @('New-WorkflowShortcut creates .cmd and .lnk files and only runs on Windows.')
    }
    ...
    if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
        Stop-WithError @(
            ...
            'Open a new PowerShell window after installing so PATH is refreshed, then run New-WorkflowShortcut again.'
        )
    }
    ...
    if ($auth.ExitCode -ne 0) {
        Stop-WithError @(
            ...
            '(the default for gh auth login), then run New-WorkflowShortcut again.'
        )
    }
    $who = Invoke-Gh @('api', 'user', '--jq', '.login')
    if ($who.ExitCode -ne 0 -or -not $who.StdOut.Trim()) {
    ...
    # --- owner ---
    Write-Step "Verifying owner '$Owner'"
    $ownerResult = Invoke-GhApi "users/$Owner"
    if (-not $ownerResult.Ok) {
        if ($ownerResult.NotFound) {
            Stop-WithError @(
                "No GitHub user or organization named '$Owner' was found.",
                'Check the -Owner value.'
            )
        }
        Stop-WithError @("Could not look up '$Owner'.", $ownerResult.Error)
    }
    $ownerInfo = $ownerResult.Data | ConvertFrom-Json
    $Owner = $ownerInfo.login
    Write-Ok "$($ownerInfo.type): $Owner"
    ...
    $Ps1Path = Join-Path $script:ActionsDir "$BaseName.ps1"
    ...
    if ($existing.Count -gt 0) {
        ...
        if (-not (Read-YesNo '    Overwrite them?' -Default $false)) {
            Stop-WithError @('Aborted. Run New-WorkflowShortcut again and choose a different shortcut name.')
        }
    }
    ...
    foreach ($dir in @($script:ActionsDir, $script:LogsDir)) {
    ...
    $runtime = $script:RuntimeTemplate
    ...
    $runtime = $runtime.Replace('__OWNER__', (ConvertTo-PsLiteral $Owner))
    ...
    try {
        ...
        $lnk.WorkingDirectory = $script:ActionsDir
    ...
    Write-Host "Run logs will be written to $($script:LogsDir)" -ForegroundColor Gray
    Write-Host ""

    # Nothing is left to clean up, so a later failure must not delete these.
    $script:CreatedFiles = @()
...
Export-ModuleMember -Function New-WorkflowShortcut
