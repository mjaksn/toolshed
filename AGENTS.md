...
- **Line endings are set per language, not per repository.** `.gitattributes`
  gives PowerShell and batch files CRLF and everything else LF, because
  PowerShell on Windows is the common case for these and some hosts are fussy
  about the shebang line in a shell script that arrived with CRLF. Do not
  normalise a file against that.
